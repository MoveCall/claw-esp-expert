...
import { PartitionAdvisor, type PartitionAdvice } from './PartitionAdvisor';
...
export class AsyncBuildManager {
    private readonly partitionAdvisor = new PartitionAdvisor();
    ...
    async build(projectPath: string, onProgress?: (msg: string) => void): Promise<BuildResult> {
        ...
        try {
            ...
            if (!await pathExists(path.join(projectPath, 'CMakeLists.txt'))) {
            ...
            await spawnCombined('idf.py', ['build'], {
                cwd: projectPath,
     ...
     * 核心诊断算法：语义化提取错误
     */
    private async diagnoseError(projectPath: string, rawLog: string): Promise<BuildResult> {
        const trimmedRawLog = rawLog.slice(-1200);

        // 模式 A: 分区表溢出 (最常见的初学者错误)
        if (
            rawLog.includes("is too large")
