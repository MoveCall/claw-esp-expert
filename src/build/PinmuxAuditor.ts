...
import { pathExists, readJsonFile } from '../common/fs';
...
export class PinmuxAuditor {
    ...
    private readonly maxAuditPin = 64;
    ...
    resolveChipTarget(target: string = 'esp32'): string {
    ...
    async loadSocRules(target: string = 'esp32'): Promise<void> {
        ...
        const candidates = [
            path.join(__dirname, `../data/soc/${resolvedTarget}.json`),
            path.join(process.cwd(), `src/data/soc/${resolvedTarget}.json`),
            path.join(process.cwd(), `dist/data/soc/${resolvedTarget}.json`)
        ];

        for (const rulePath of candidates) {
            if (await pathExists(rulePath)) {
                this.rules = await readJsonFile(rulePath);
                return;
            }
        }
    ...
    async auditSourceCode(projectPath: string): Promise<AuditResult[]> {
        ...
        const sourceRoots = [path.join(projectPath, 'main'), path.join(projectPath, 'components')];
     ...
     */
    private checkFlashPins(occurrences: PinOccurrence[]): AuditResult[] {
        const results: AuditResult[] = [];
     ...
     */
    private checkInputOnlyPins(sources: SourceFile[], occurrences: PinOccurrence[]): AuditResult[] {
        const results: AuditResult[] = [];
     ...
     */
    private checkStrappingPins(occurrences: PinOccurrence[]): AuditResult[] {
        const results: AuditResult[] = [];
     ...
     */
    private checkAdc2WifiConflict(sources: SourceFile[], code: string, occurrences: PinOccurrence[]): AuditResult[] {
        const results: AuditResult[] = [];
        ...
        for (const pin of adc2Pins) {
            const occurrence = this.findFirstOccurrence(
                occurrences.filter((item) => {
                    if (item.pin !== pin) return false;
     ...
     */
    private collectPinOccurrences(sources: SourceFile[], symbols: Map<string, SymbolValue>): PinOccurrence[] {
        const occurrences: PinOccurrence[] = [];
    ...
    private buildSymbolTable(sources: SourceFile[]): Map<string, SymbolValue> {
    ...
    private extractPinsFromLine(line: string, symbols: Map<string, SymbolValue>): number[] {
    ...
    private extractSymbolDefinitions(line: string, symbols: Map<string, SymbolValue>): Array<{ name: string; pin: number }> {
        const definitions: Array<{ name: string; pin: number }> = [];
        const patterns = [
    ...
    private resolvePinExpression(expression: string, symbols: Map<string, SymbolValue>): number | null {
    ...
    private looksLikePinRelatedIdentifier(name: string): boolean {
        return /(gpio|pin|io|sda|scl|mosi|miso|clk|cs|tx|rx|rst|reset|irq|int|busy)/i.test(name);
    }

    private isOutputContext(source: SourceFile, line: number): boolean {
    ...
    private isAdcContext(source: SourceFile, line: number): boolean {
        const snippet = this.getSnippet(source, line, 3).join('\n');
        return /\badc\b|adc2_get_raw|adc_oneshot|ADC_UNIT_2|adc_channel_t/i.test(snippet);
    }

    private getSnippet(source: SourceFile, centerLine: number, radius: number): string[] {
    ...
    private findFirstOccurrence(occurrences: PinOccurrence[], pin: number): PinOccurrence | undefined {
    ...
    private dedupeResults(results: AuditResult[]): AuditResult[] {
        const seen = new Set<string>();
