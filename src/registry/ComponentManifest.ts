...
export class ComponentManifestManager {
    ...
    private normalizeManifest(value: string): string {
        const normalized = value.replace(/\r\n/g, '\n').trimEnd();
        return normalized;
    }

    private appendDependenciesBlock(manifestText: string, suggestion: ComponentSuggestion): string {
        const block = [
            'dependencies:',
    ...
    private insertDependency(lines: string[], dependencyBlockEndLine: number, suggestion: ComponentSuggestion): string {
        const insertion = [
            `  ${suggestion.dependency}:`,
            `    version: "${suggestion.version}"`
        ];
    ...
    private replaceDependencyVersion(lines: string[], entry: DependencyEntry, nextVersion: string): string {
        const nextLines = [...lines];
    ...
    private findDependenciesBlock(lines: string[]): {
        exists: boolean;
        startLine: number;
        endLine: number;
        ...
        let current: DependencyEntry | null = null;
