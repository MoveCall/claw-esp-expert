...
import { MonitorAnalyzer, type MonitorAnalysisResult } from './MonitorAnalyzer';
import { pathExists } from '../common/fs';
import { spawnCombined } from '../common/process';
...
export class FlashMonitorManager {
    private readonly monitorAnalyzer = new MonitorAnalyzer();
    private readonly defaultTimeoutMs = 15000;
    private readonly logTailLines = 16;
    ...
    async run(args: {
        ...
        if (!await pathExists(path.join(args.projectPath, 'CMakeLists.txt'))) {
            return {
                status: 'ENV_ERROR',
        ...
        try {
            const result = await spawnCombined('idf.py', command, {
    ...
    private extractLogTail(log: string): string[] {
        return log
            .replace(/\r\n/g, '\n')
            .split('\n')
    ...
    private classifyFailure(error: any, log: string): FlashMonitorResult['failureCategory'] {
    ...
    private buildFailureReason(error: any, log: string): string {
    ...
    private buildStageSummary(log: string, analysis: MonitorAnalysisResult, error: any): string {
        const category = this.classifyFailure(error, log);
